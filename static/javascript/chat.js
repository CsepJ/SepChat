...
document.addEventListener("DOMContentLoaded", () => {
    ...
    socket.on('chat', function(data){
        ...
        if(data.id != id){
            $("ul#chatlog").append(`<li name="chat">
                    <div class="sender" name="chat"><img name="chat" class="profile" id="${data.id}" src="${data.profile}" alt="${data.id}" title="${data.id}" style="filter: ${senderImgFilter};"/><span name="chat" class="name">${data.sender}</span><span name="chat" class="time" style="color: ${timeColor};">${data.time.toLocaleString("ko")}</span></div>
            ...
            $("ul#chatlog").append(`<li name="userchat">
                    <div name="chat" class="sender"><img name="chat" class="profile" id="${data.id}" src="/image/me.png" alt="${data.id}" title="${data.id}"/ style="filter: ${senderImgFilter};"><span name="chat" class="name">${data.sender}</span><span name="chat" class="time" style="color: ${timeColor};">${data.time.toLocaleString("ko")}</span></div>
                    <div name="chat" class="message">${data.message}</div>
                </li>`);
    ...
    $('html').click(function(e) {
        if($(e.target).attr("class") == "profile"){
            ...
            pop_up.setAttribute("id", $(e.target).attr("id"));
            pop_up.setAttribute("class", "userinfo");
            document.getElementById("popup").appendChild(pop_up);
        }else if($(e.target).attr("class") != "profile" && $(e.target).attr("class") != "userinfo"){
